<!-- arlette:documentation commit=ee63ce2 date=2026-07-14 engine=codex -->
...
SB Racing Web transpose dans un navigateur l’expérience essentielle de *SB Pro Racing* : course sur ovale, vue du dessus, peloton contrôlé par IA et stratégie de stands. Le MVP actuel propose un circuit, jusqu’à quinze voitures, une physique arcade avec tête-à-queue, le carburant, les pneus, les dégâts, les collisions, un cycle jour-nuit et un classement chronométré. L’interface française fonctionne au clavier, à la manette ou au tactile, avec plein écran, sans backend, compte utilisateur ni ressource audiovisuelle externe. Le tutoriel, les records et la qualification ne sont pas livrés ; leurs entrées restent désactivées ou absentes.
...
| `.github/` | Exécute sur `main` ou manuellement le lint, les onze suites, le build Node 20 puis le déploiement de `dist` sur GitHub Pages. |
...
| `src/` | Répartit 54 fichiers TypeScript entre onze domaines de simulation, rendu, interface, données et tests. |
...
| `vitest.config.ts` | Exécute sous Node les onze suites correspondant au motif `src/tests/**/*.test.ts`. |
...
- Les unions `RacePhase`, `RaceState` et `PitPhase` modélisent des machines à états explicites ; les stands suivent cinq phases, tandis que `DayNightPhase` décrit quatre phases dérivées de la progression du meneur.
...
- `PlayerController` fusionne clavier, première manette et boutons tactiles par maximum pour les pédales et somme bornée pour la direction.
...
- Le dictionnaire `fr` centralise 125 libellés et `t()` réalise l’interpolation par placeholders `{nom}`.
...
- Gamepad API lit stick, croix, gâchettes et boutons de la première manette ; `navigator.getGamepads()` déclenche facultativement une vibration `dual-rumble` lors des collisions.
...
- **Circuit** — `CLASSIC_OVAL` construit un monde `2400 × 1400`, quatre checkpoints, quinze grilles, quinze stands et neuf projecteurs.
- **Traductions** — `fr` associe 125 clés aux libellés français ; `t()` interpole les paramètres avec `replaceAll`.
...
- **Circuit** — `ensureTrackTexture` dessine un canvas déterministe unique avec piste, stands, tribunes, public, infield et neuf pylônes.
...
- **Stockage** — `loadSettings` assainit volume, booléens et neuf réglages, dont les courses bornées entre 20 et 200 tours ; `saveSettings` tolère les erreurs de `localStorage`.
...
- **Configuration** — `RaceSettings` expose neuf paramètres, bornés à deux–quinze voitures et 20–200 tours, initialisés à dix voitures, vingt tours et niveaux `normal`.
- **Orchestration** — `RaceController` séquence la simulation fixe, publie huit événements et clôt les attardés après `75 s`.
- **Cycle lumineux** — `DayNightSystem` dérive `day`, `dusk`, `night` et `dawn` aux progressions `0,2`, `0,4` et `0,75`, avec transitions `smoothstep`.
...
- **Menu** — `MenuScene` propose six entrées, dont tutoriel et records désactivés avec la mention « À venir ».
- **Commandes** — `ControlsScene` aligne trois colonnes décrivant clavier, manette et tactile, puis revient au menu par `MenuList`.
- **Configuration** — `SetupScene` modifie neuf paramètres, quatre presets `20/30/50/100`, vingt couleurs et les bornes persistées.
- **Options** — `OptionsScene` règle le volume par pas de `10 %`, persiste immédiatement le son et accepte le retour manette.
- **Course** — `RaceScene` rend cycle jour-nuit, véhicules, éclairages, caméra, particules, stands, pause, commandes tactiles et simulation à pas fixe.
...
- **Simulation** — `damage`, `fuel`, `tires`, `pit`, `strategy` et `spin` valident usure, pannes, réparations, stratégie IA, dérapage et tête-à-queue.
- **Course** — `lapTracker`, `ranking` et `dayNight` valident anti-triche, progression, arrivée, écarts, formatage et phases lumineuses.
- **Infrastructure** — `storage` et `track` valident assainissement local, surfaces, murs, progression et conversions.
- **Couverture** — Les onze suites regroupent soixante-treize cas exécutés par Vitest sous Node.
...
- **Tactile** — `TouchControls` expose six boutons circulaires multi-touch pour direction, pédales, pause et plein écran sur les appareils compatibles.
...
- **État** — `Vehicle` centralise identité, cinématique, ressources, chronométrage, cinq états de course, cinq phases de stands et l’accumulateur `spinHeat`.
- **Fabrique** — `createRaceField` tire jusqu’à quatorze IA, attribue couleurs, numéros, grille et stands distincts.
- **Joueur** — `PlayerController` verrouille le départ puis fusionne clavier, première manette et source tactile en commandes normalisées.
- **IA** — `AIController` suit trois lignes, anticipe six distances, dépasse, récupère, évite les toupies et planifie ses arrêts selon la durée.
